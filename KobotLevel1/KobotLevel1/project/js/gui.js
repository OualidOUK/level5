...
class Gui {
    constructor() {
        ...
        this.asynchronizer = new Blockly.Gamepad.Asynchronizer(
            ...
            function (id) {
                ...
                // this.chargers_count = document.getElementsByClassName
    ...
    load(id) {
        ...
        if(id) {
            this.id = id;
            // Swal.fire({
            //     title: "Instruction for level " + str,
            //     icon: "info",
            ...
            //   dots[id-1].innerHTML = dots[id-1].value;

              if ("chargers" in levels[id-1]) { 

                this.listChargers = document.getElementsByClassName("chargers_" + id)
                this.chargersCount = document.getElementsByClassName("chargerscount_"+id)

                for (let index = 0; index < this.chargersCount.length; index++) {
                    this.chargersCount[index].textContent = levels[id - 1].chargers[index].count
        ...
        // gmelvl1.style.display = this.id == 1 ? 'block' : "none";
        // gmelvl2.style.display = this.id == 2 ? 'block' : "none";
        // gmelvl3.style.display = this.id == 3 ? 'block' : "none"; 
        // gmelvl4.style.display = this.id == 4 ? 'block' : "none"; 
        // gmelvl4.style.display = this.id == 4 ? 'block' : "none"; 
        gmelvl5.style.display = this.id == 5 ? 'block' : "none";

        Blockly.getMainWorkspace().addChangeListener(() => {
            let capacity = Blockly.getMainWorkspace().remainingCapacity()
            // console.log(Blockly.getMainWorkspace().options)
            if (capacity != Infinity) {
                // show capacity div
                
                // document.getElementById('capacity').style.display = 'block'
                // document.getElementById('capacityNumber').innerHTML = capacity
                document.getElementById('CountBlocks').innerHTML= Blockly.getMainWorkspace().options.maxBlocks - capacity
                document.getElementById('maxblocks').innerHTML = Blockly.getMainWorkspace().options.maxBlocks
            } else {
                // hide capacity div
